...
public class CompleteBOTriggerPB {
    private static final Logger logger = Logger.getLogger(CompleteBOTriggerPB.class);
    ...
    public static void main(String[] args) {
        ...
        try {
         ...
         // Wait for 0.5 minute
            ...
            triggerAPI(); // Call triggerAPI without passing any arguments
        } catch (Exception e) {
    ...
    private static void triggerAPI() {
        try {
            Document triggerAgentPBdoc = createCachedGroupsDocument();
            logger.debug("Input XML: " + documentToString(triggerAgentPBdoc));
          
            triggerAgentAPI(triggerAgentPBdoc);
        } catch (Exception e) {
    ...
    private static Document createCachedGroupsDocument() throws Exception {
        ...
        Document triggerAgentPBdoc = docBuilder.newDocument();
        
        // Log to check if the method is called
        logger.debug("Creating TriggerAgent Document");        

        Element triggerAgentPB = triggerAgentPBdoc.createElement("TriggerAgentPB");
        triggerAgentPB.setAttribute("CriteriaId", "BO_COMP_SCHEDULE_PB");
        
        // Log to check if the element is added to the document
        logger.debug("TriggerAgentPB element created");
        
        // Append the element to the document
        triggerAgentPBdoc.appendChild(triggerAgentPB);

        return triggerAgentPBdoc;
    }
    ...
    private static void triggerAgentAPI(Document triggerAgentPBdoc) throws YFSException, RemoteException {
        if (api != null && env != null) {
            api.invoke(env, "triggerAgent", triggerAgentPBdoc); ////-->used for api
